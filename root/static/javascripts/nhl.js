...
jQuery.getJSON("[% c.uri_for( '/static/testdata/nhl.json' ) %] ", function(data) {
...
function renderTradeChart(jsondata){
	...
	**/
	...
	var tradeSizes = jQuery.map(trades,function(value,index){
...
function fade(opacity,teams) {
   return function(g, i) {
	...
	var items = jQuery.grep(d3.select("#trades").selectAll("path").data(),function (item) {
	    return jQuery.inArray(item, paths.data()) < 0;
	});
    //get the team a and team b of the trade
    var involvedTeams = jQuery.map(items,function(value,index){
    ...
    d3.select("#trades").selectAll("image")
         .filter(function(d){
				return jQuery.inArray(d,involvedTeams) == -1;
