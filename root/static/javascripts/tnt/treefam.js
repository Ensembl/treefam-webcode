...
var treefam_theme = function() {
    ...
    var theme = function(ta, div, tree_url, ensembl_species_url) {
    ...
    var get_highest_val = function(node, prop) {
		...
		var deploy_vis = function(tree_obj) {
			...
			function rightRoundedRect(args) {
				...
				var end = match_end ? "M" + (x+width) + "," + (y+height) : "v" + height;
				var start = match_start ? "M" + x + "," + y : "v" + -height;
    		...
    		// 				.attr("d", function(d){
    		// 					var match_start = (d.hmm_start > 10)? false:true;
    		// 					var match_end = (d.hmm_start > 10)? false:true;
...
var get_pfam_match_boundaries = function(nodes) {
	...
	for (var i = 0; i < nodes.length; i++) {
		...
		if (bs) {
			...
			var domain_name = bs[j].name
			if( domain2color[domain_name] === undefined ) {
								domain2color[domain_name] = p_taxa(i_domain % p_taxa.range().length);
								i_domain++;
							}
				console.log("color "+domain_name+" as "+domain2color[domain_name])
				this_boundaries.push({
					start: bs[j].ali_from,
					end: bs[j].ali_to,
					hmm_start: bs[j].hmm_from,
					hmm_end: bs[j].hmm_to,
					color: domain2color[domain_name]
				});
		...
		// 	});
			...
			var id = leaf.id();
			var name;
			if(!leaf.data().children && !leaf.data()._children && leaf.data().id && leaf.data().id.accession){
				name = leaf.data().id.accession;
			}
