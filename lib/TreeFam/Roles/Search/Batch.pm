...
package TreeFam::Roles::Search::Batch;
...
sub batch : Chained( 'search' )
  ...
  # set the queue
  $c->stash->{job_type} = 'treefam_batch';
...
sub queue_rfam : Private {
  ...
  unless ( $c->forward('queue_search_transaction') ) {
    ...
    $c->stash->{rest}->{error} = 'There was an error when registering your TreeFam search.';
