...
# very big = > 200
  ...
  if ( defined $active_letters ) {
    ...
    foreach my $family ( @families ) {
    	#use Data::Dumper;
      $first_letter = uc( substr( $family->stable_id, 0, 1 ) );
      ...
      foreach my $range (keys(%range_hash)){
      	my ($start,$end) = ($range_hash{$range}{'min'},$range_hash{$range}{'max'});
      	if ($start <= $number && $number <= $end) {
      		$family_sizes->{family_range}->{$range} += 1;
      	}
      }
      $family_sizes->{families}->{$number} = 1;
