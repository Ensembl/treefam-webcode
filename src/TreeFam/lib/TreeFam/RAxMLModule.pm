...
##my $baseUrl = 'http://wwwdev.ebi.ac.uk/Tools/services/rest/raxml_epa';
...
sub start {
    ...
    my $alignment = $tool_params{'alignment'};    # Input alignment file
    my $newick    = $tool_params{'newick'};       # Input Newick tree file
    my $algorithm = $tool_params{'algorithm'};    # Algorithm to use
    ...
    if (
         !( $params{'polljob'} || $params{'resultTypes'} || $params{'status'} || $params{'params'} || $params{'paramDetail'} )
         && !(
               ( defined( $ARGV[0] ) && defined( $ARGV[1] ) ) || (    defined( $params{'alignment'} )
