...
sub start
{
	...
	my $sequence		= $params{'sequence'};      # Input sequence
	my $fastaalign		= $params{'fastaalign'};    # Input FASTA alignment
	...
	if ( !( $params{'polljob'} || $params{'resultTypes'} || $params{'status'} || $params{'params'} || $params{'paramDetail'}) && !( defined( $ARGV[0] ) || defined( $params{'sequence'} ) ))
	{
...
=cut
#This is a temporary function ... until we get a REST call from systems.
sub run_mafft
{
	my ($arg_ref)		= @_;
	...
	open(ALN,">/nfs/production/xfam/treefam/projects/tree_insertion/tmp/ref_aln.aln");
	print ALN "${$ref_alignment}";
	close(ALN);

	open(QRY,">/nfs/production/xfam/treefam/projects/tree_insertion/tmp/query.fasta");
	print QRY ">QRY\n";
	print QRY "${$query_fasta}";
	close(QRY);

	print "Running mafft:\n";
	system("/ebi/extserv/bin/mafft-6.850/bin/mafft --auto --quiet --add /nfs/production/xfam/treefam/projects/tree_insertion/tmp/query.fasta /nfs/production/xfam/treefam/projects/tree_insertion/tmp/ref_aln.aln > /nfs/production/xfam/treefam/projects/tree_insertion/tmp/mafft.aln");

	${$mafft_alignment} = `cat /nfs/production/xfam/treefam/projects/tree_insertion/tmp/mafft.aln`;
}
1;
