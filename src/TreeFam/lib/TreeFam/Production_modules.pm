...
sub read_hgnc_mappings 
{
	my ( $arg_ref ) = @_;
	my $file    = $arg_ref->{ file };
	my $hgnc_mapping_href       = $arg_ref->{ hgnc_mapping_hash };
	my $hgnc_mappingID_href       = $arg_ref->{ hgnc_mappingID_hash };
		
	open my $fileH, "<", $file or die "Could not open $file\n";
	while(<$fileH>){
		next if /^$/;
		next if /^HGNC ID/;
		my ($id,$symbol,$name,$status,$prev_symbol,$synonyms,$chrom, $acc,$ensembl,$ref_seq) = split(/\t/,$_);
		print "id:$id,symbol:$symbol,name:$name,status:$status,prev:$prev_symbol,syn:$synonyms,chrom:$chrom,acc:$acc,ensembl:$ensembl,ref:$ref_seq\n" if $id eq "HGNC:24712";	
		#print "$synonyms\n" if $id eq "HGNC:1101" ;
		my $hgnc_id = $id;
		$hgnc_id =~ s/HGNC://;
		...
		foreach my $syn(split(",",$synonyms)){
			$syn =~ s/^\s//g;
			push(@{$hgnc_mapping_href->{$hgnc_id}},$syn); 
			print "saving synonym: $hgnc_id -> $syn\n";
		}
		push(@{$hgnc_mapping_href->{$hgnc_id}},$name); 
		$hgnc_mappingID_href->{$hgnc_id} = $symbol; 
		print "saving symbol: $hgnc_id -> $symbol\n";

		#last if $hgnc_id == "8350";
	#die "here\n";
	}	
	print "Found ".keys(%{$hgnc_mapping_href})." and ".keys(%{$hgnc_mappingID_href})."\n";
	close $fileH || die "Could not close $file\n";
}
...
sub convert_nhx_to_unique_ids
{
	my ( $arg_ref )   = @_;
	my $superhash     = $arg_ref->{ superhash };
	my $sequence_href = $arg_ref->{ sequences_href };
	my $treefam_name  = $arg_ref->{ treefam_name };
	my $nhx_tree      = $arg_ref->{ nhx_tree };
	my $newick_tree   = NHX2newick( $nhx_tree );
	my $replaced_tree = taxonomic( $nhx_tree );

	#print "original tree  is : ".$nhx_tree."\n";
	#print "replaced_tree is : ".$replaced_tree."\n";
	$superhash->{ 'nhx_unique_tree' } = $replaced_tree;
...
sub taxonomic
{
	...
	my @tok = split( /\)/, $nhx_tree );
	foreach my $split ( @tok )
	{

		#print ">>$split\n";
		if ( $split =~ /^\w/ )
		{
			my @taxon = split( /\:|\[/, $split );
			$TAXON_count{ $taxon[ 0 ] }++;

			#print "|$taxon[0]_$TAXON_count{$taxon[0]}\n";
			#print "|$taxon[0]|\n";
			$nhx_tree =~ s/\Q$taxon[0]:/$taxon[0]\_$TAXON_count{$taxon[0]}:/;
		}
	}
...
sub get_pruned_trees
{
	...
	my $newick_tree = $superhash->{ 'full_unique_tree_newick' };
	#print "before pruning the tree, should be unique and internal node names: ".$newick_tree. "\n";

	#die "printed newick\n";
	my @ids2keep_model_tree;
	my @ids2keep_wormbase_tree;
	my @ids2remove_model_tree;
	my @ids2remove_wormbase_tree;
	my %allowed_species_ids_model = (
									  "10116"  => 1,
									  "10090"  => 1,
									  "9606"   => 1,
									  "9031"   => 1,
	...
	my %allowed_species_ids_wormbase = (
										 "10116"  => 1,
										 "10090"  => 1,
										 "9606"   => 1,
										 "9031"   => 1,
	...
	foreach my $leaf ( @{ $all_leaves } )
	{
		...
		push( @ids2keep_model_tree,    $stable_id ) if ( exists $allowed_species_ids_model{ $taxon } );
		push( @ids2keep_wormbase_tree, $stable_id ) if ( exists $allowed_species_ids_wormbase{ $taxon } );
		push( @ids2remove_model_tree,  $stable_id ) if ( not exists $allowed_species_ids_model{ $taxon } );

	}
	print "Found model: " . scalar( @ids2keep_model_tree ) . " and wormbase: " . scalar( @ids2keep_wormbase_tree ) . "";
	...
	my $wormbase_tree = parse(
							   -format => 'newick',
							   -string => $newick_tree
	)->first;
	my $ret_tree_model    = $model_tree->keep_tips( \@ids2keep_model_tree );
	my $ret_tree_wormbase = $wormbase_tree->keep_tips( \@ids2keep_wormbase_tree );

	#if ( ref( $ret_tree_model ) eq ref( $ret_tree_wormbase ) )
	#{
	#	print "INTERESTING: model tree and wormbase tree are the same!!!\n";
	#}

	#my $ret_tree = $root->keep_nodes_by_taxon_ids(\@ids2keep);
	
	my $model_tree_string;
	$model_tree->visit_depth_first(
        '-pre_daughter'   => sub { $model_tree_string .=  '('             },     
        '-post_daughter'  => sub { $model_tree_string .=  ')'             },     
        '-in'             => sub { $model_tree_string .=  shift->get_name },
        '-pre_sister'     => sub { $model_tree_string .=  ','             },     
 );
	$model_tree_string .= ';';
	my $wormbase_tree_string;
	$wormbase_tree->visit_depth_first(
        '-pre_daughter'   => sub { $wormbase_tree_string .=  '('             },     
        '-post_daughter'  => sub { $wormbase_tree_string .=  ')'             },     
        '-in'             => sub { $wormbase_tree_string .=  shift->get_name },
        '-pre_sister'     => sub { $wormbase_tree_string .=  ','             },     
 );
	$wormbase_tree_string .= ';';



	print "\nfound model tree: $model_tree_string\n";
	print "\nfound wormbase tree: $wormbase_tree_string\n";
	$superhash->{ 'model_tree_newick' }    = $model_tree_string;
	$superhash->{ 'wormbase_tree_newick' } = $wormbase_tree_string;

	#my $pruned_tree = $ret_tree->nhx_format();
	#print Dumper $pruned_tree;


	return ( $superhash->{ 'model_tree_newick' } ne '' ) ? 1 : undef;
...
### do we need to clean old entry?
	...
	my @tags_to_delete = (
						   "fam_description",      "fam_n_full",          "hgnc",       "fam_n_seed",
						   "fam_full_description", "fam_symbol",          "fasta_aln",  "homologs_array_json",
						   "numspecies",           "pfam",                "treenhx",    "treephyloxml",
						   "tree_image_png",       "sequence_array_json", "taxa_count", "wikigene",
						   "numSequences",         "json_tree",           "minimal_species_tree_json", "full_tree_json",
						   "model_tree_json", "wormbase_tree_json"
	);
...
#my $result = traverse({"node"=> $tree->get_root, "sequence_href" => $sequence_href});
	...
	sub species_tree_traverse
	{
		...
		if ( my @children = @{ $node->get_children } )
		{
			$result->{ 'children' } = [ map { species_tree_traverse( { "node" => $_, "sequence_href" => $sequence_href, "species2gene" => $species2gene, } ) } @children ];
		}
...
sub newick2json
{
	...
	my $input_tree           = "$treefam_name.in.newick";
	my $species2gene         = $arg_ref->{ species2gene };
	my $model_tree_newick    = $superhash->{ 'model_tree_newick' };
	my $wormbase_tree_newick = $superhash->{ 'wormbase_tree_newick' };
...
# read taxon2species
	...
	# read full nhx tree and save annotation
	if (
		 !read_tree_io(
						{
	...
	#die "check full gene tree newick here: $output_tree\n";
...
# loop over all possible trees we want to create
# e.g. full, model, ?
	my $proj = parse(
					  '-format'     => 'newick',
					  '-file'       => $output_tree,
	...
	$superhash->{ "full_tree_json" } = JSON->new->encode( $result );


	#print "now do the same but for model tree\n";
	my $model_tree = parse(
							-format => 'newick',
							-string => $model_tree_newick
	)->first;
	my $model_tree_result = traverse(
									  {
										"node"               => $model_tree->get_root,
										"sequence_href"      => $sequence_href,
										"seqIDLength"        => \%seqIDLength,
										"species2gene"       => $species2gene,
										"taxid2species_hash" => \%taxid2species_hash,
	...
	#print "finished looking at model tree\n";
	# save trees for later
	$superhash->{ "model_tree_json" } = JSON->new->encode( $model_tree_result );
	#print "$model_tree_result\n";
	#print "" . $superhash->{ "model_tree_json" } . "\n";
	#print "now do the same but for model tree\n";
	my $wormbase_tree = parse(
							   -format => 'newick',
							   -string => $wormbase_tree_newick
	)->first;
	my $wormbase_tree_result = traverse(
										 {
										   "node"               => $wormbase_tree->get_root,
										   "sequence_href"      => $sequence_href,
										   "seqIDLength"        => \%seqIDLength,
										   "species2gene"       => $species2gene,
										   "taxid2species_hash" => \%taxid2species_hash,
	...
	#print "finished looking at model tree\n";
	# save trees for later
	$superhash->{ "wormbase_tree_json" } = JSON->new->encode( $wormbase_tree_result );
	#print "$wormbase_tree_result\n";
	#print "" . $superhash->{ "wormbase_tree_json" } . "\n";
	unlink( $output_tree ) if -e $output_tree;
	...
	sub traverse
	{
			...
			foreach my $pfam_hit ( keys( %{ $sequence_href->{ $node_name }{ pfam_hits } } ) )
			{
				...
				foreach my $domain_hit ( keys( %{ $sequence_href->{ $node_name }{ pfam_hits }{ $pfam_hit } } ) )
				{
					my ( $id, $name, $a_start, $a_end, $evalue ) = (
																	 $sequence_href->{ $node_name }{ pfam_hits }{ $pfam_hit }{ $domain_hit }{ hmm_name },
		...
		my $result = {
			...
			branch_length => ( exists $allNodesMapping{ $node_name }{ branch_length } ) ? $allNodesMapping{ $node_name }{ branch_length } : "N/A",
			common_name => ( exists $taxid2species_href->{ $allNodesMapping{ $node_name }{ T } }{ common_name } )
			? $taxid2species_href->{ $allNodesMapping{ $node_name }{ T } }{ common_name }
			: "no common name",
			...
			swissprot_gene_name => ( exists $sequence_href->{ $node_name }{ swissprot_hits }{ 'gene_name' } ) ? $sequence_href->{ $node_name }{ swissprot_hits }{ 'gene_name' }
			: "N/A",
			swissprot_protein_name => ( exists $sequence_href->{ $node_name }{ swissprot_hits }{ 'protein_name' } )
			? $sequence_href->{ $node_name }{ swissprot_hits }{ 'protein_name' }
			: "N/A",
			...
			foreach my $node ( @nodes )
			{
				...
				if ( $node->is_Leaf )
				{
					...
					foreach ( @tags )
					{
						...
						$leafNodesMapping{ $name }{ branch_length } = $branch_length;
						$allNodesMapping{ $name }{ branch_length }  = $branch_length;
				...
				else
				{
					...
					$internalNodesMapping{ $newID }{ 'branch_length' } = $branch_length;
					$allNodesMapping{ $newID }{ 'branch_length' }      = $branch_length;
...
sub transfer_annotations
{
	...
	foreach my $ensembl_id ( keys( %swissprot_sequences ) )
	{
		...
		my ( $gene_name, $protein_name ) = ( $swissprot_sequences{ $ensembl_id }{ 'gene_names' }, $swissprot_sequences{ $ensembl_id }{ 'protein_names' } );
...
sub get_sequence_annotations
{
	...
	foreach my $ensembl_prot_id ( keys( %{ $sequences_hash } ) )
	{
		...
		$sequences_hash->{ $ensembl_prot_id }{ "pfam_hits" } =
		  &get_pfam_hits( { "id" => $ensembl_prot_id, "file_to_search" => $pfam_file, "pfam_counts" => \%{ $ext_counts->{ "pfam_counts" } }, "db_adaptor" => $db_adaptor } );
		...
		{
			$sequences_hash->{ $ensembl_prot_id }{ "wikigene_hits" } =
			  &get_wikigene_hits( { "id" => $gene4prot, "file_to_search" => $wikigene_file, "db_adaptor" => $db_adaptor } );
			...
			$sequences_hash->{ $ensembl_prot_id }{ "go_hits" } =
			  &get_go_hits( { "id" => $ensembl_prot_id, "file_to_search" => $pfam_file, "pfam_counts" => \%{ $ext_counts->{ "pfam_counts" } }, "db_adaptor" => $db_adaptor } );
		}
	}
	foreach my $ensembl_prot_id ( keys( %{ $sequences_hash } ) )
	{
		...
		push( @seq_array,
			  ( exists $sequences_hash->{ $ensembl_prot_id }{ "taxon_name" } && $sequences_hash->{ $ensembl_prot_id }{ "taxon_name" } ne '' )
			  ? $sequences_hash->{ $ensembl_prot_id }{ "taxon_name" }
			  : "NaN" );
		push( @seq_array, $ensembl_prot_id );
		push( @seq_array,
			  ( exists $sequences_hash->{ $ensembl_prot_id }{ "hmmer_hits" } && $sequences_hash->{ $ensembl_prot_id }{ "hmmer_hits" } ne '' )
			  ? $sequences_hash->{ $ensembl_prot_id }{ "hmmer_hits" }
			  : "No hits" );
		...
		if (    !exists( $sequences_hash->{ $ensembl_prot_id }{ "description" } )
			 || !defined( $sequences_hash->{ $ensembl_prot_id }{ "description" } )
			 || $sequences_hash->{ $ensembl_prot_id }{ "description" } eq '' )
		{
...
sub get_family_annotations
{
	...
	my ( $acc, $symbol, $desc, $seed, $full, $date1, $date2, $type, $full_desc, $comment ) =
	  &get_previous_treefam_annotation( { "id" => $tree->stable_id, "file_to_search" => $previous_treefam_file } );
...
sub get_tree_nhx
{
	...
	my $newick_tree   = NHX2newick( $c->{ 'treeNhx' });
	$c->{ 'full_tree_newick' } = $newick_tree;
	
	# the newick_format does not seem to work -> ignores internal node names
	#$c->{ 'full_tree_newick' } = $root_of_tree->newick_format;
...
sub get_hgnc_hits
{
	...
	my $db_adaptor        = $arg_ref->{ db_adaptor };
	my $extID2seq_sth_ext = $db_adaptor->prepare( 'select hgnc_id, symbol, approved_name, fam_tag, fam_description from hgnc_mapping where ensembl_gene_id = ?' );
	...
	my %hgnc_hits;
### Parse
	while ( my ( $hgnc_id, $app_symbol, $app_description, $fam_tag,$fam_description ) = $extID2seq_sth->fetchrow_array() )
	{
...
sub read_domain_info_file
{
	...
	{
		chomp( $line );
		my ( $seqID, $alignment_start, $alignment_end, $envelope_start, $envelope_end, $hmm_acc, $hmm_name, $type, $hmm_start, $hmm_end, $hmm_length, $bit_score, $E_value,
...
sub _create_dummy_architecture
{
	...
	{
		my ( $from, $to, $confidence, $hmm_name ) = (
													  $domain_info_href->{ $domain_number }{ 'alignment_start' },
...
sub _create_dummy_taxonomy
{
	...
	return "<scientific_name>$scientific_name</scientific_name>
